...
contract LiquidateLoan is FlashLoanReceiverBase, Ownable {
    ...
    constructor(ILendingPoolAddressesProvider _addressProvider, IUniswapV2Router02 _uniswapV2Router) FlashLoanReceiverBase(_addressProvider) {
        _provider = _addressProvider;
        lendingPoolAddr = _provider.getLendingPool();
     ...
     */
    function executeOperation(address[] calldata assets, uint256[] calldata amounts, uint256[] calldata premiums, address initiator, bytes calldata params)
        external
        override
        returns (bool)
    {
    ...
    function calcProfits(uint256 _balance, uint256 _loanAmount, uint256 _loanFee) private pure returns (uint256) {
    ...
    function swapToBarrowedAsset(address asset_from, address asset_to, uint256 amountOutMin, address[] memory swapPath) public {
        ...
        try uniswapV2Router.swapExactTokensForTokens(amountToTrade, amountOutMin, swapPath, address(this), deadline) {}
    ...
    */
    function executeFlashLoans(address _assetToLiquidate, uint256 _flashAmt, address _collateral, address _userToLiquidate, uint256 _amountOutMin, address[] memory _swapPath)
        public
        onlyOwner
    {
